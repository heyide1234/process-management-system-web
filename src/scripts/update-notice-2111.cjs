    ...
    <div class="word-document">
      <table class="doc-table" border="1">
        ...
        <tr>
          ...
          <td class="label-cell">通报/普查文件名称</td>
          <td class="value-cell"><el-input v-model="form.documentName" :disabled="!isEditMode" class="plain-input" /></td>
        </tr>
        <tr>
...
const exportToWord = () => {
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>技术通报</title><style>td{padding: 5px;}</style></head><body>' +
    '<table border="1" style="border-collapse: collapse; width: 100%;">' +
    '<tr><td colspan="4" style="text-align: center; font-weight: bold;">技术通报/专项普查内容</td></tr>' +
    '<tr><td style="width: 120px; font-weight: bold;">序号</td><td colspan="3">' + form.serialNumber + '</td></tr>' +
    '<tr><td style="font-weight: bold;">通报/普查文件名称</td><td colspan="3">' + form.documentName + '</td></tr>' +
    '<tr><td style="font-weight: bold;">发文单位</td><td>' + form.issuingUnit + '</td><td style="font-weight: bold;">发文号</td><td>' + form.documentNumber + '</td></tr>' +
    '<tr><td style="font-weight: bold;">执行等级</td><td>' + execLevelText + '</td><td style="font-weight: bold;">落实方式</td><td>' + implMethodText + '</td></tr>' +
    '<tr><td style="font-weight: bold;">工厂编号</td><td>' + form.factoryNumber + '</td><td style="font-weight: bold;">专业</td><td>' + form.major + '</td></tr>' +
    '<tr><td style="font-weight: bold;">发文日期</td><td>' + (form.issueDate ? new Date(form.issueDate).toLocaleDateString('zh-CN') : '') + '</td>' +
    '<td style="font-weight: bold;">收文日期</td><td>' + (form.receiptDate ? new Date(form.receiptDate).toLocaleDateString('zh-CN') : '') + '</td></tr>' +
    '<tr><td style="font-weight: bold;">内容摘要</td><td colspan="3" style="height: 80px;">' + form.contentSummary + '</td></tr>' +
    '<tr><td style="font-weight: bold;">涉及范围</td><td colspan="3" style="height: 80px;">' + form.scope + '</td></tr>' +
    '<tr><td style="font-weight: bold;">落实方法</td><td colspan="3" style="height: 80px;">' + form.implementation + '</td></tr>' +
    '<tr><td style="font-weight: bold;">变更记录</td><td colspan="3" style="height: 80px;">' + form.changeRecords + '</td></tr>' +
    '<tr><td style="font-weight: bold;">落实小结</td><td colspan="3" style="height: 80px;">' + form.summary + '</td></tr>' +
    '<tr><td style="font-weight: bold;">落实负责人</td><td>' + form.responsiblePerson + '</td>' +
    '<td style="font-weight: bold;">落实期限</td><td>' + (form.deadline ? new Date(form.deadline).toLocaleDateString('zh-CN') : '') + '</td></tr>' +
    '<tr><td style="font-weight: bold;">收文人</td><td>' + form.receiver + '</td><td style="font-weight: bold;">质控安监室主任</td><td>' + form.director + '</td></tr>' +
    '</table></body></html>'
