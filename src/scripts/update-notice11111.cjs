...
const exportToWord = () => {
  ...
  const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>技术通报</title></head><body>' +
    ...
    '<tr><td style="width: 120px;">序号</td><td colspan="3">' + form.serialNumber + '</td></tr>' +
    '<tr><td>通报/普查文件名称</td><td colspan="3">' + form.documentName + '</td></tr>' +
    '<tr><td>发文单位</td><td>' + form.issuingUnit + '</td><td>发文号</td><td>' + form.documentNumber + '</td></tr>' +
    '<tr><td>执行等级</td><td>' + execLevelText + '</td><td>落实方式</td><td>' + implMethodText + '</td></tr>' +
    '<tr><td>工厂编号</td><td>' + form.factoryNumber + '</td><td>专业</td><td>' + form.major + '</td></tr>' +
    '<tr><td>发文日期</td><td>' + (form.issueDate ? new Date(form.issueDate).toLocaleDateString('zh-CN') : '') + '</td>' +
    '<td>收文日期</td><td>' + (form.receiptDate ? new Date(form.receiptDate).toLocaleDateString('zh-CN') : '') + '</td></tr>' +
    '<tr><td>内容摘要</td><td colspan="3" style="height: 100px;">' + form.contentSummary + '</td></tr>' +
    '<tr><td>涉及范围</td><td colspan="3" style="height: 100px;">' + form.scope + '</td></tr>' +
    '<tr><td>落实方法</td><td colspan="3" style="height: 160px;">' + form.implementation + '</td></tr>' +
    '<tr><td>变更记录</td><td colspan="3" style="height: 120px;">' + form.changeRecords + '</td></tr>' +
    '<tr><td>落实小结</td><td colspan="3" style="height: 80px;">' + form.summary + '</td></tr>' +
    '<tr><td>落实负责人</td><td>' + form.responsiblePerson + '</td>' +
    '<td>落实期限</td><td>' + (form.deadline ? new Date(form.deadline).toLocaleDateString('zh-CN') : '') + '</td></tr>' +
    '<tr><td>收文人</td><td>' + form.receiver + '</td><td>质控安监室主任</td><td>' + form.director + '</td></tr>' +
    '</table></body></html>'
